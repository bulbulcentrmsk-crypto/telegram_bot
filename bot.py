...
async def cmd_start(message: types.Message):
    ...
    if user.role == 'admin':
        ...
        welcome_text = f"👋 Здравствуйте, {message.from_user.first_name}! Добро пожаловать! Оставьте заявку или посмотрите наши центры."
...
async def view_referrals(message: types.Message):
    ...
    if user and user.role == 'agent':
        ...
        if referrals:
            text = "📊 Твои рефералы:\n\n" + "\n".join([f"• {r.first_name} ({r.registered_at.strftime('%d.%m.%Y')})" for r in referrals])
        else:
...
async def agent_requests(message: types.Message):
    ...
    if user and user.role == 'agent':
        ...
        if agent:
            ...
            if requests:
                text = "📋 Твои заявки:\n\n" + "\n".join([f"• {r.full_name} - {r.status}" for r in requests[:5]])
            else:
                text = "Заявок нет."
        else:
            text = "Ты не агент."
...
async def show_centers(message: types.Message):
    ...
    if not centers:
        ...
        text = "🏥 Центры:\n\n" + "\n\n".join([f"**{c.name}**\n📍 {c.address}\n📞 {c.phone}\n{c.description}" for c in centers])
...
async def admin_centers_menu(callback: types.CallbackQuery):
    ...
    if centers:
        text += "\n".join([f"{'✅' if c.is_active else '❌'} {c.name}" for c in centers])
    else:
        text += "Центров нет."
...
async def list_centers(callback: types.CallbackQuery):
    ...
    for center in centers:
        keyboard.add(InlineKeyboardButton(f"{'✅' if center.is_active else '❌'} {center.name}", callback_data=f'center_{center.id}'))
...
@dp.callback_query_handler(lambda c: c.data.startswith('center_') and c.data != 'admin_centers', user_id=config.ADMIN_IDS)
...
async def edit_name_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(center_id=center_id, field='name')
...
async def edit_addr_start(callback: types.CallbackQuery, state: FSMContext):
    center_id = int(callback.data.split('_')[2])
    await state.update_data(center_id=center_id, field='address')
...
async def edit_phone_start(callback: types.CallbackQuery, state: FSMContext):
    center_id = int(callback.data.split('_')[2])
    await state.update_data(center_id=center_id, field='phone')
...
async def edit_desc_start(callback: types.CallbackQuery, state: FSMContext):
    center_id = int(callback.data.split('_')[2])
    await state.update_data(center_id=center_id, field='description')
...
async def save_address(message: types.Message, state: FSMContext):
    ...
    if center:
        ...
        session.commit()
        await message.answer(f"✅ Адрес изменен на: {message.text}")
    session.close()
    await state.finish()
...
async def save_phone(message: types.Message, state: FSMContext):
    ...
    if center:
        ...
        session.commit()
        await message.answer(f"✅ Телефон изменен на: {message.text}")
    session.close()
    await state.finish()
...
async def delete_center_confirm(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup().add(
        ...
        InlineKeyboardButton('❌ Нет', callback_data=f'center_{center_id}')
    )
    await callback.message.edit_text("⚠️ Удалить центр?", reply_markup=keyboard)
    await callback.answer()
...
async def delete_center(callback: types.CallbackQuery):
    ...
    if center:
        ...
        session.commit()
        await callback.message.edit_text(f"✅ Центр '{name}' удален")
    session.close()
    await callback.answer()
...
async def add_center_start(callback: types.CallbackQuery):
    await callback.message.answer("Введите название центра:")
...
async def add_center_description(message: types.Message, state: FSMContext):
    session = Session()
    data = await state.get_data()
    center = Center(name=data['name'], address=data['address'], phone=data['phone'], description=message.text, is_active=True)
    session.add(center)
    session.commit()
    await message.answer(f"✅ Центр '{data['name']}' добавлен!")
    session.close()
    await state.finish()
...
async def sync_centers(callback: types.CallbackQuery):
    ...
    for cid, info in config.CENTERS_INFO.items():
        ...
        center = Center(name=name, address=address, phone=phone, description=info, is_active=True)
        session.add(center)
    session.commit()
    await callback.message.edit_text("✅ Центры синхронизированы")
    await callback.answer()
    session.close()
...
async def admin_agents(message: types.Message):
    keyboard = InlineKeyboardMarkup(row_width=2).add(
        ...
        InlineKeyboardButton('📊 Статистика', callback_data='agents_stats')
    )
...
async def admin_stats(message: types.Message):
    session = Session()
    text = f"📊 **СТАТИСТИКА**\n\n👥 Всего: {session.query(User).count()}\n👤 Агентов: {session.query(User).filter_by(role='agent').count()}\n👥 Рефералов: {session.query(User).filter_by(role='referral').count()}\n📝 Заявок: {session.query(Request).count()}"
    await message.answer(text, parse_mode='Markdown')
    session.close()
...
@dp.callback_query_handler(lambda c: c.data == 'agents_stats', user_id=config.ADMIN_IDS)
async def agents_stats(callback: types.CallbackQuery):
    session = Session()
    agents = session.query(Agent).all()
    text = "📊 **Статистика агентов:**\n\n"
    for agent in agents:
